<?php
namespace Drupal\rti_hindi\Controller;
...
class RtihindiController {
    public function rti_hindi_listing() {
      
      
      global $base_url;
        // $this->addLink('dsadsa');
        $query =\Drupal::database()->select('rti');
...
$variables['home_page']['tendr'] = array();	$i=1;
		...
		if($vl->pdf!='no')
		{
			...
			if($pdf!='')
			{
		       $filename =  $pdf->getFilename();
				...
				if($q[2]=='hi')
				{
				  $ss= strip_tags($vl->rti_title_hi).'@'.$filename;
			...
			$rid_child= $vl->id;
			$quchild =\Drupal::database()->select('rti_sublink');
			$res_child = $quchild->fields('rti_sublink')->condition('rti_id', $rid_child)->execute()->fetchAll();	
			...
			foreach($res_child as $titchild)
			{
				...
				if($pdf_child!=''){
					...
					if($q[2] =='hi'){
					// echo "<pre>data-"; print_r($titchild); exit;
					$ss_child[]= strip_tags($titchild->rti_child_hi).'@'.$filename_child;
    ...
    $variables['home_page']['tendr'][] =  array(
		...
		//'tname_child' => $exp_child,
			...
			}else{$title= 'सूचना का अधिकार';$titlemain= 'सूचना का विवरण (आरटीआई)';
			 $variables['home_page']['tendr'][] =  array(
		'sr' => $i++,
        'rtiname' => wordwrap(ltrim($val->rti_name_hi," "),100,"\n"),
		'tname' => $exp,
		//'tname_child' => $exp_child,
		//'pdf' => $expp,
	 ...
	 return array('#theme' => 'rti_hindi_template','#titlemain' => $titlemain,'#title' => $title,'#name'=>$variables['home_page']['tendr']) ;		
