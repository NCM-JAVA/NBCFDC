...
class Editnfobc_child extends FormBase{
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
      ...
      $select = \Drupal::database()->select('nfobc_sublink');
      $select->fields('nfobc_sublink');
	...
	$allimg= $results->pdf;	   
	   ...
	   $form['nfobc_sublink_name'] = array(
        '#prefix'=>'<div class="col-md-4">NFOBC Sublink Name<span class="form-required" title="This field is required."></span>',
        '#suffix'=>'</div>',
        '#type' => 'textarea',
        '#default_value'	 => $results->nfobc_child_title,
        '#attributes'=>array('required'=>'required','class'=>array('form-control'))
        );
	   
       $form['nfobc_sublink_name_hi'] = array(
        '#prefix'=>'<div class="col-md-4">NFOBC Sublink Name In Hindi<span class="form-required" title="This field is required."></span>',
        '#suffix'=>'</div>',
        '#type' => 'textarea',
        '#default_value'	 => $results->nfobc_child_hi,
         '#attributes'=>array('required'=>'required','class'=>array('form-control'))
        );
 ...
 $file->save();	        
           ...
           $q= end(explode('/',$current_uri));
            ...
            $fields = array(
             ...
             $query = (\Drupal :: database () -> update ('nfobc_sublink'));
