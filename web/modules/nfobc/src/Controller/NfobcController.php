...
class NfobcController {
    public function nfobc_listing() {
...
$variables['home_page']['tendr'] = array();	$i=1;
		...
		if($vl->pdf!='no')
		{
			...
			if($pdf!='')
			{
		       $filename =  $pdf->getFilename();
				...
				{
					$ss= strip_tags($vl->nfobc_title).'@'.$filename;				
			...
			foreach($res_child as $titchild)
			{
				...
				if($pdf_child!=''){
					...
					$ss_child[]= strip_tags($titchild->nfobc_child_title).'@'.$filename_child;
    ...
    $variables['home_page']['tendr'][] =  array(
		...
		//'tname_child' => $exp_child,
			...
			}else{$title= 'NFOBC';$titlemain= 'NFOBC';
			 $variables['home_page']['tendr'][] =  array(
		'sr' => $i++,
        'rtiname' => wordwrap(ltrim($val->nfobc_name," "),100,"\n"),
		'tname' => $exp,
		//'tname_child' => $exp_child,
		//'pdf' => $expp,
	 ...
	 return array('#theme' => 'nfobc_template','#titlemain' => $titlemain,'#title' => $title,'#name'=>$variables['home_page']['tendr']) ;		
